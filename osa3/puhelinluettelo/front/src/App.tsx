...
const App = () => {
    ...
    function handleSubmit(event: FormEvent) {
        ...
        if (existingPerson) {
            if (window.confirm(`${existingPerson.name} is already added to phonebook, replace the old number with a new one?`)) {
                ...
                PersonService.update(existingPerson._id, existingPerson).then(() => {
                    PersonService.getAll().then(r => {
                        setPersons(r.data);
        ...
        if (newPerson._id === "") {
            newPerson._id = String(persons.length + 1);
        }
    ...
    function handleDelete(event: FormEvent, p: Person) {
        event.preventDefault()
        if (window.confirm(`Delete ${p.name} ${p._id}`)) {
            PersonService.drop(p._id).then(() => {
                PersonService.getAll().then(r => {
                    setPersons(r.data);
